...
GvChat::GvChat(const std::string& url, const std::string& client_id, const std::string& client_secret,
               const std::string& version, const std::string& user_agent)
: _url(url), _client_id(client_id), _client_secret(client_secret), _version(version), _user_agent(user_agent)
...
{
  if (isSleeping()) { // when sleeping, just write directly to the log
    basic_mud_log("GV CHAT: %s", msg.c_str());
  } else { // when awake, queue log messages
...
void GvChat::processLog()
{
  ...
  while (!_logQueue.empty()) {
    mudlog(NRM, LVL_IMMORT, TRUE, "GV CHAT: %s", _logQueue.front().c_str());
    _logQueue.pop();
  }
}
...
void GvChat::start()
{
  ...
  _webSocket.setOnMessageCallback([this](const WebSocketMessagePtr& msg) {
    std::stringstream ss;
    if (msg->type == ix::WebSocketMessageType::Open) { // if we're connected, authenticate
      ss << "Connected to: " << msg->openInfo.uri << std::endl;
      log(ss.str());
      authenticate();
    } else if (msg->type == ix::WebSocketMessageType::Close) { // if we're disconnected,
      // report why
      ss << _url << " disconnected!"
         << " code " << msg->closeInfo.code << " reason " << msg->closeInfo.reason;
      log(ss.str());
      _authenticated = false;
    } else if (msg->type == ix::WebSocketMessageType::Message) { // decode incoming messages
      // immediately if sleeping
      if (isSleeping()) {
        decodeMessage(msg->str);
      } else { // queue messages if the game is active
        std::lock_guard<std::mutex> lock(_msgMutex);
        _receivedQueue.push(msg->str);
      }
    } else if (msg->type == ix::WebSocketMessageType::Error) { // log errors
      ...
      log(ss.str());
      _authenticated = false;
    } else { // log unknown websocket responses
...
void GvChat::eventAuthenticate(json j)
{
  ...
  if (j["status"] == "success") {
    ss << " unicode: " << j["payload"]["unicode"] << " version: " << j["payload"]["version"];
    _authenticated = true;
    updateGamesStatus();
    updatePlayersStatus();
    processSends();
...
void GvChat::eventHeartbeat(json j)
{
  ...
  if (!isSleeping()) { // build list of players for heartbeat
...
{
  std::stringstream ss;
  ss << "Server restarting... downtime: " << j["payload"]["downtime"];
  log(ss.str());
}
...
void GvChat::eventSignIn(json j)
{
  ...
  if (!j["payload"].is_object()) { // notify a character with given reference of sign in
    if (isSleeping()) {
      return;
    }
    ...
    send_to_char(ch, "%sYou have been signed in to the Grapevine chat network.%s\r\n", CBYEL(ch, C_NRM),
                 CCNRM(ch, C_NRM));
    updateGamesStatus(GVGAME);
    return;
  } else { // we have a payload, a player signed in from another game
    ...
    for (auto& i : descriptor_list) {
      if (STATE(i) == CON_PLAYING && i->character && can_hear_check(i->character)
        ...
        send_to_char(i->character, "%sGV: %s has signed in to %s%s%s.\r\n", CCYEL(i->character, C_NRM), name.c_str(),
                     CBBLU(i->character, C_NRM), game.c_str(), CCNRM(i->character, C_NRM));
      }
    }
  }
}
...
void GvChat::eventSignOut(json j)
{
  ...
  updateGamesStatus(game);
  if (!playersOnline[game].empty()) {
    for (auto p = playersOnline[game].begin(); p != playersOnline[game].end();) {
      if (*p == name)
  ...
  for (auto& i : descriptor_list) {
    if (STATE(i) == CON_PLAYING && i->character && can_hear_check(i->character)
        && PLT_FLAGGED(i->character, PLT_GVPLAYERS)) {
      send_to_char(i->character, "%sGV: %s has signed out of %s%s%s.\r\n", CCYEL(i->character, C_NRM), name.c_str(),
                   CBBLU(i->character, C_NRM), game.c_str(), CCNRM(i->character, C_NRM));
    }
  }
  return;
...
void GvChat::eventSuccess(json j)
{
  ...
  if (j["event"] == "tells/send")
    send_to_char(ch, "%s%s%s", CCRED(ch, C_NRM), a.second.c_str(), CCNRM(ch, C_NRM));
  else if (j["event"] == "channels/send")
    send_to_char(ch, "%s%s%s", CCYEL(ch, C_NRM), a.second.c_str(), CCNRM(ch, C_NRM));
}
...
void GvChat::eventTellsReceive(json j)
{
  ...
  for (auto& i : descriptor_list) { // loop through all online players
    if (STATE(i) == CON_PLAYING && i->character && can_hear_check(i->character)
        && !str_cmp(GET_NAME(i->character),
                    to_name.c_str())) { // if we're able to hear and our name
      // matches, send tell
      send_to_char(i->character, "%sGV: %s%s@%s%s%s tells you '%s'%s", CCYEL(i->character, C_NRM),
                   CCRED(i->character, C_NRM), from_name.c_str(), CBBLU(i->character, C_NRM), from_game.c_str(),
                   CCRED(i->character, C_NRM), message.c_str(), CCNRM(i->character, C_NRM));
    }
  }
}
...
void GvChat::eventGamesConnect(json j, bool connected)
{
  ...
  for (auto& i : descriptor_list) {
    if (STATE(i) != CON_PLAYING || !i->character || !PLT_FLAGGED(i->character, PLT_GVGAMES))
      continue;

    if (connected)
      send_to_char(i->character, "%sGV: Game %s%s%s has connected to Grapevine.%s\r\n", CCYEL(i->character, C_NRM),
                   CBBLU(i->character, C_NRM), game.c_str(), CCYEL(i->character, C_NRM), CCNRM(i->character, C_NRM));
    else
      send_to_char(i->character, "%sGV: Game %s%s%s has disconnected from Grapevine.%s\r\n", CCYEL(i->character, C_NRM),
                   CBBLU(i->character, C_NRM), game.c_str(), CCYEL(i->character, C_NRM), CCNRM(i->character, C_NRM));
  }
}
...
void GvChat::eventGamesStatus(json j)
{
  ...
  if (_clearGames) { // clear the game list on the first response
    gameStatus.clear();
    _clearGames = false;
  }
  gameStatus[game] = p;
...
{
  std::string event = j["event"];
  if (!isReady() || (event != "authenticate" && !_authenticated)) { // queue sent messages if not connected
    // or authenticated
    std::lock_guard<std::mutex> lock(_sendMutex);
...
void GvChat::playerSignOut(chPtr ch)
{
  ...
  if (!playersOnline[GVGAME].empty()) {
    for (auto p = playersOnline[GVGAME].begin(); p != playersOnline[GVGAME].end();) {
      if (*p == name)
...
{
  json j;
  std::string aMessage("GV [" + *ch->player.gvChannel + "] " + std::string(GET_NAME(ch)) + "@" + GVGAME + ": " + message
                       + "\r\n");
  auto g = newAction(ch, aMessage);
  j["event"] = "channels/send";
  j["ref"] = g;
  j["payload"] = {{"channel", *ch->player.gvChannel}, {"name", GET_NAME(ch)}, {"message", message}};
  sendMessage(j);
}

// send tell to player in a different game
void GvChat::sendTell(chPtr ch, const std::string& to_name, const std::string& to_game, const std::string& message)
{
  json j;
  std::string aMessage(std::string(CCYEL(ch, C_NRM)) + "GV: " + CCRED(ch, C_NRM) + "You tell " + to_name + "@"
                       + CBBLU(ch, C_NRM) + to_game + CCRED(ch, C_NRM) + ", '" + message + "'" + CCNRM(ch, C_NRM)
                       + "\r\n");
  auto g = newAction(ch, aMessage);
  ...
  if (game.empty()) // all games
  {
    ...
    for (const auto& [g, j] : GvChat->gameStatus) {
      ...
      if (!count) {
        ...
        ss << CCYEL(ch, C_NRM) << count << " player" << (count == 1 ? "" : "s") << " online on " << CBBLU(ch, C_NRM)
           << g << CCNRM(ch, C_NRM) << ":";
      for (const auto& p : GvChat->playersOnline[g]) {
        // put quotes around names with spaces
    ...
    sprintf(buf, "player%s in Grapevine game %s%s%s%s", (count == 1) ? "" : "s", CBBLU(ch, C_NRM), game.c_str(),
            CCNRM(ch, C_NRM), count ? ": " : "");
    for (auto& p : GvChat->playersOnline[game]) {
      sprintf(buf, "%s%s%s%s ", buf, CCYEL(ch, C_NRM), p.c_str(), CCNRM(ch, C_NRM));
    }
    if (count) {
...
ACMD(do_gvgame)
{
  ...
  if (game.empty()) // all games
  {
    ...
    for (const auto [g, j] : GvChat->gameStatus) {
      ss << "(" << CBBLU(ch, C_NRM) << g << CCNRM(ch, C_NRM) << ") ";
  ...
  } else // specific game
  {
    ...
    ss << CBYEL(ch, C_NRM) << "GV Game: " << CCNRM(ch, C_NRM) << "(" << CBBLU(ch, C_NRM) << g["game"].get<std::string>()
       << CCNRM(ch, C_NRM) << ") ";
    ...
    if (g["homepage_url"].is_string())
      ss << CCYEL(ch, C_NRM) << "URL: " << CCNRM(ch, C_NRM) << g["homepage_url"].get<std::string>() << endl;
    if (g["user_agent"].is_string())
      ss << CCYEL(ch, C_NRM) << "User Agent: " << CCNRM(ch, C_NRM) << g["user_agent"].get<std::string>() << endl;
    if (g["user_agent_repo_url"].is_string())
      ss << CCYEL(ch, C_NRM) << "Repo URL: " << CCNRM(ch, C_NRM) << g["user_agent_repo_url"].get<std::string>() << endl;
    ss << CCYEL(ch, C_NRM) << "Players Online: " << CCNRM(ch, C_NRM) << player_online_count << endl;
    ...
    if (g["connections"].is_array()) {
      ...
      for (auto& con : g["connections"]) {
        if (con["type"] == "web")
          ss << endl << CCYEL(ch, C_NRM) << "    Web: " << CCNRM(ch, C_NRM) << con["url"].get<std::string>();
        else if (con["type"] == "telnet")
          ss << endl
             << CCYEL(ch, C_NRM) << "    Telnet: " << CCNRM(ch, C_NRM) << con["host"].get<std::string>() << ":"
             << con["port"];
        else if (con["type"] == "secure telnet")
          ss << endl
             << CCYEL(ch, C_NRM) << "    Secure Telnet: " << CCNRM(ch, C_NRM) << con["host"].get<std::string>() << ":"
             << con["port"];
      }
    }
...
ACMD(do_gvchannel)
{
  ...
  if (channel == "gossip" || channel == "testing") {
    ...
    send_to_char(ch, "%sGV: Channel set to %s%s", CCYEL(ch, C_NRM), arg1, CCNRM(ch, C_NRM));
    return;
  }
  ...
  if (!message.empty()) {
    message = emojicpp::emojize(message);
    ...
    for (auto& i : descriptor_list) {
      if (STATE(i) == CON_PLAYING && i->character && can_hear_check(i->character) && i && i->character != ch) {
        send_to_char(CCYEL(i->character, C_NRM), i->character);
        act(message.c_str(), FALSE, ch, 0, i->character, 0, 0, TO_VICT | TO_SLEEP);
...
ACMD(do_gvset)
{
  ...
  if (!*buf) {
    send_to_char(ch, "Usage: gvset <games | players | channels>\r\n");
    return;
  }
  ...
  if (is_abbrev(buf, "games")) {
    ...
    send_to_char(ch, "Grapevine game status messages are now %s.", ONOFF(PLT_FLAGGED(ch, PLT_GVGAMES)));
  } else if (is_abbrev(buf, "players")) {
    // toggle players
    TOGGLE_BIT(PLT_FLAGS(ch), PLT_GVPLAYERS);
    send_to_char(ch, "Grapevine player status messages are now %s.", ONOFF(PLT_FLAGGED(ch, PLT_GVPLAYERS)));
  } else if (is_abbrev(buf, "channels")) {
    // toggle channel messages
    TOGGLE_BIT(PLT_FLAGS(ch), PLT_GVNOCHANNELS);
    send_to_char(ch, "Grapevine channel messages are now %s.", ONOFF(!PLT_FLAGGED(ch, PLT_GVNOCHANNELS)));
  } else {
    send_to_char(ch, "Usage: gvset <games | players | channels>\r\n");
    return;
  }
...
// emojize from https://github.com/99xt/emojicpp
namespace emojicpp {
  ...
  for (int i = 0; i < sLen; i++) {
    if (s[i] == *L":") {
      if (index == -1) {
        ...
        // uncomment to see what text is replaced, might be good for debugging
...
// display and/or search for emojis
ACMD(do_emoji)
{
  std::stringstream ss;
  ...
  if (!*buf) { // display all emoji if the user hasn't specified a search
    ss << "The following emoji are available to you:" << endl;
    for (const auto [e, j] : emojicpp::EMOJIS) {
      ss << e << " = " << j << endl;
    }
  } else { // search for a specific emoji
    ss << "The following emoji that contain " << buf << " are available to you:" << endl;

    for (const auto [e, j] : emojicpp::EMOJIS) {
      if (e.find(buf) != string::npos)
        ss << e << " = " << j << endl;
    }
  }

  // have to strlcpy to buf cause page_string doesn't like const
  strlcpy(buf, ss.str().c_str(), sizeof(buf));
  page_string(ch->desc, buf, true);
}
